...
In a 0D setup, we did not find evidence for computational advantages of the IA approach over the DA variant.
In a spatially explicit setup, performance gains 
%for simulating a small number of phytoplankton groups 
may be possible, but would require modifying the physical-flux calculations to account for spatial differences in $Q$ between model grid cells.
\end{abstract}
...
\section{Conclusion}
%Take home messages 
%What was achieved, what is left for future work.\\
%Remaining challanges to achieve an efficient model that can mimic the expensive full quota model.
Accounting for the variabilities in cellular composition of phytoplankton is required for a realistic representation of the nutrient cycles, which, however, results in additional state variables, and consequently additional computational costs. As a remedy, \citet{Smith2016} proposed the Instantaneous Acclimation (IA) approach, which approximates the variabilities in the cellular composition that was described by a Dynamic Acclimation (DA) described by \citet{Pahlowetal13}, but without using an additional state variable for N. As long as only one of C or N are fully resolved (i.e., their mass balance is globally closed), this approach, without requiring additional flux terms, is fully conservative, and thus, can be used, e.g., for ecologically oriented questions \citep[e.g.][]{Kerimoglu2021}. Here, we provided a formally consistent and complete explanation of how the mass balance can be nearly re-established. Through various tests in 0D setups, we demonstrated that under stable environmental conditions, the fully explicit model can be closely reproduced, but that during transients, differences in specific predictions between the IA and DA variants can emerge, and that mass balance can be slightly compromised. Here we were concerned only with variabilities, and a generalization of this approach to account for spatial variabilities requires extension of this framework, which can be aimed in future work. In our 0D setups, we did not find evidence for improved computational efficiency, however gains in spatially expicit setups are possible, given that the number of state variables to be transported is known to be a costly process.

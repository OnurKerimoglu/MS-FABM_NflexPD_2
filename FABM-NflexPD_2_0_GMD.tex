...
\begin{itemize}
 ...
 \item comparison of the behavior of the previous and current C- and N- based model versions in a 0D setup \onur{not sure if needed? Not sure also whether the two versions will produce quantitatively identical results because of the approximations and potential omission of a term in the N-based version (see below)}
...
\onur{In the N-based version, we had (K21-N, Eq.~5): $F_{DIN-Phy_{\text{N}}} = V_N \cdot Phy_{\text{C}}$. i.e., excluding the $\frac{\text{d} Q_\text{N}}{\text{d} t}$ term in Eq.~\ref{eq:dphyNdt3}. As we discussed, this apparent difference (apparent, because maybe there is a mistake in the notation?) doesn't violate the internal consistency of the N-based version, but it would be great if we could explain what this really is. For instance, is this a `real deviation' in model formulations that contributes (among other sources) to quantitative differences in results?
}

Given that the terms $\hat{V}_\text{N}$ and $\hat{\mu}_{net}$ used for calculating $Q_\text{N}$ (Eq.~\ref{eq:Qopt}) are respectively functions of DIN (Eq.~16 in K21-N) and daytime average irradiance, $\bar{I}$ (Eqs.~20-22 in K21-N), the total time derivative of $Q_\text{N}$ in Eq.~\ref{eq:dphyNdt3} can be computed as the sum of partial derivatives with respect to DIN and $\bar{I}$ using chain rule: 
...
\end{equation}
following \citet{Smith2016}, defining $Z = \frac{2}{Q_\text{N,0}{({\hat{\mu}_{\text{net}}}/{\hat{V}_\text{N}}+\zeta_{\text{N}})}}$ in Eq.~\ref{eq:Qopt}, partial derivatives of $Q_\text{N}$ with respect to DIN and $\bar{I}$ are found as:
\begin{flalign}
 \frac{\partial Q_\text{N}}{\partial \text{DIN}} &= \frac{-Q_\text{N,0}}{4 \cdot Z \cdot \sqrt{Z\cdot(1+Z)}} \frac{\partial Z}{\partial \text{DIN}}\\
 \frac{\partial Q_\text{N}}{\partial \bar{I}}  &= \frac{-Q_\text{N,0}}{4 \cdot Z \cdot \sqrt{Z\cdot(1+Z)}}  \frac{\partial Z}{\partial \bar{I}}
\end{flalign}
and the partial derivatives of $Z$ with respect to DIN and $\bar{I}$:
\begin{flalign}
 \frac{\partial Z}{\partial \text{DIN}} &= \frac{-\hat{\mu}_{net} \cdot Q_\text{N,0}}{2 \cdot \hat{V}_\text{N} \cdot \text{DIN}} \left( 1 - \frac{\hat{V}_\text{N}}{\hat{V}_\text{N,0}} - \frac{\hat{V}_\text{N}}{\sqrt{\hat{V}_\text{N,0} \cdot \hat{A}_\text{N,0} \cdot \text{DIN}}} \right) \\
 \frac{\partial Z}{\partial \bar{I}} &= \frac{Q_\text{N,0} \cdot \alpha \cdot \hat{\theta}}{2 \cdot \hat{V}_\text{N} \cdot \text{DIN} \color{red}{(?)}} \left ( 1 - L_\text{I} \right) 
\end{flalign}
\onur{check: 1) Should we have DIN in the denominotor of $\frac{\partial Z}{\partial \bar{I}}$? I have a note both on the printed appendix and on the code that this should be omitted. 2) replacement of $\hat{\mu}_g$ (in S16, $\hat{\mu}^I$) with $\hat{\mu}_{net}$ may require revision of last equation}\\
where $\alpha$ is a model parameter (initial Chl-specific slope of P-I curve, Table~3 in K21-N); $\hat{\theta}$ is the optimal chlorophyll density in the chloroplast (Eq.~26 in K21-N) and $L_I$ is light limitation (Eq.~22 in K21-N). 

The last two unknown terms in Eq.~\ref{eq:dqdt} are the changes in $\bar{I}$ and DIN over time. The former, $\frac{\text{d} \bar{I}}{\text{d} t}$ is approximated by the discrete backward difference between the current ($t=i$) and previous ($t=i-1$) value of $\bar{I}$, divided by the size of the integration time step i.e., $\frac{\text{d} \bar{I}}{\text{d} t} \sim \frac{\bar{I}_{i} - \bar{I}_{i-1}}{\Delta t}$.  The latter, $\frac{\text{d} DIN}{\text{d} t}$ is eliminated by expanding Eq.~\ref{eq:sdin} based on Eqs.~\ref{eq:dphyNdt}-\ref{eq:dqdt}: 

...
% Table
\begin{table*}[htb]
  \caption{ Descriptions, values and units of model parameters regarding phytoplankton growth. All parameter values are taken from within the published range \citep{Pahlowetal13,Smith2016}, without particular reference to species.\label{T:params}}
  \begin{tabular}{l l c l l}
    \tophline
    Term/Symbol         & Definition                        & Value     & Unit                                  &Used by\\
    \middlehline
    $\hat{\mu}_0$   & Potential maximum growth rate         & 5.0       & \unit{d^{-1}}                         & all\\
    $Q_0|_{\textrm{ESD}=1\mu\textrm{m}}$  & Subsistence quota                     & 0.039     & \unit{mmolN\ molC^{-1}}               & IA, DA\\
    $\alpha_{Q_0}$  & Allometric scaling coefficient of $Q_0$       & ?     & \unit{mmolN\ molC^{-1}}               & IA, DA\\
    $\hat{A}_0|_{\textrm{ESD}=1\mu\textrm{m}}$     & Potential maximum nutrient affinity   & 0.15       & \unit{m^3\ mmolC^{-1}\ d^{-1}}        & IA, DA\\
    $\alpha_{A_0}$  & Allometric scaling coefficient of $\hat{A}_0$        & ?     & \unit{mmolN\ molC^{-1}}               & IA, DA\\
    $\hat{V}_0|_{\textrm{ESD}=1\mu\textrm{m}}$     & Potential maximum N uptake rate       & 5.0       & \unit{molN\ molC^{-1}\ d^{-1}}        & IA, DA\\
    $\alpha_{V_0}$  & Allometric scaling coefficient of $\hat{V}_0$        & ?     & \unit{mmolN\ molC^{-1}}               & IA, DA\\
    $\alpha$           & Chl-specific slope of P-I curve    & 1.0       & \unit{m^2\ E\ molC gChl^{-1}\ d^{-1}} & all\\
    $R^{\text{Chl}}_{\text{M}}$     & Cost of chlorophyll maintenance   & 0.1       & \unit{d^{-1}}             & all\\
    $\zeta_{\text{Chl}}$   & Cost of chlorophyll synthesis  & 0.5       & \unit{mmolC\ gChl^{-1}}               & all\\
    $\zeta_{\text{N}}$     & Cost of N uptake               & 0.6       & \unit{molC\ molN^{-1}}                & all\\
    \bottomhline
  \end{tabular}
...
\subsubsection{0D setup (T1-T3)}
    ...
    \begin{figure}[ht!]
    \includegraphics[width=8cm,trim=0mm 54mm 0cm 0mm, clip]{figures/2022-03-22a_Ld1_T20_ThatVar_PARint_vs_PARext/0D-Highlat_wconst_lext_Ld1_T20_CbasedIA_modular_24h_vs_0D-Highlat_wconst_lint_Ld1_T20_CbasedIA_modular_24h_cont_abio0_1y.png}
...
\begin{figure}[ht!]
  \includegraphics[width=8cm,trim=0mm 0mm 0cm 0mm, clip]{figures/2022-03-22d_Ldvar_Tvar_ThatVar_PARext_IA_vs_DA/0D-Highlat_wconst_lext_Ldvar_Tvar_DIN015_CbasedIA_modular_24h_vs_0D-Highlat_wconst_lext_Ldvar_Tvar_DIN015_CbasedDA_modular_24h_cont_abio0_2y.png}
  \caption{Daily average irradiance and its temporal derivative, as extracted from the simulation outputs generated for T1. PAR:N (solid blue line) is the model version where the temporal derivative of irradiance is approximated numerically; PAR:A (dashed orange line): both irradiance and its temporal derivative are calculated analytically.\label{f.T2env}}
\end{figure}
...
\caption{Scaling of simulation time with number of phytoplankton instances being simulated.\label{f.T3res}}
\end{figure}

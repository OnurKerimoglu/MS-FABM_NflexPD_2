...
\begin{document}
\title{FABM-NflexPD 2.0: An instantaneous acclimation approach for modelling carbon, nitrogen [and phosphorus?] cycles}
...
\begin{description}
 ...
 %\item [T4] \onur{todo, I doubt that this is needed} comparison of the behavior of the previous and current C- and N- based model versions in a 0D setup
...
$Q_{\text{N}}$ is assumed to instantaneously adjust to a balanced-growth optimum value, as determined by the nutrient uptake in the protoplast, $\hat{V}_{\text{N}}$ and net photosynthesis in the chloroplast, $\hat{\mu}_{net}$ (Eq.~10 in K21):
...
\begin{flalign}\label{eq:sdin2}
  ...
  = F_{\text{DON}-\text{DIN}} - V_{\text{N}} - \text{Phy}_{\text{C}} \cdot \left(\frac{\partial Q_{\text{N}}}{\partial \text{DIN}} \frac{\text{d} \text{DIN}}{\text{d} t}
    + \frac{\partial Q_{\text{N}}}{\partial \bar{I}} \frac{\text{d} \bar{I}}{\text{d} t} + \frac{\partial Q_{\text{N}}}{\partial L_{\text{D}}} \frac{\text{d} L_{\text{D}}}{\text{d} t} \right) \\
  \intertext{and reorganizing:}
%intermediate step:
% \begin{equation}\label{eq:sdin2.5}
%  \frac{\text{d}\text{DIN}}{\text{d}t} \left( 1+\text{Phy}_{\text{C}}\frac{\partial Q_{\text{N}}}{\partial \text{DIN}} \right) = F_{\text{DON}-\text{DIN}} - \text{Phy}_{\text{C}} \left(V_N +  \frac{\partial Q_{\text{N}}}{\partial \bar{I}} \frac{\text{d} \bar{I}}{\text{d} t} \right)\\
% \end{equation}
\label{eq:sdin3}
\frac{\text{d}\text{DIN}}{\text{d}t} &= \frac{\displaystyle F_{\text{DON}-\text{DIN}} - V_{\text{N}} - \text{Phy}_{\text{C}} \cdot
  \left( \frac{\partial Q_{\text{N}}}{\partial \bar{I}} \frac{\text{d} \bar{I}}{\text{d} t}
    + \frac{\partial Q_{\text{N}}}{\partial L_{\text{D}}} \frac{\text{d} L_{\text{D}}}{\text{d} t} \right)}%
...
% \end{equation}
% where, assuming balanced growth \citep{Burmaster1979}, i.e., $V_N = Q_N \cdot \mu$  (K21, Eq.~6) the first term on the right hand side of Eq.~\eqref{eq:dphyNdt2} can be replaced with $V_N \cdot \text{Phy}_{\text{C}}$, i.e., 
...
%  \frac{\text{d}\text{DIN}}{\text{d}t} = F_{\text{DON}-\text{DIN}} - \text{Phy}_{\text{C}} \left(V_N + \frac{\partial Q_{\text{N}}}{\partial \text{DIN}} \frac{\text{d} \text{DIN}}{\text{d} t} +  \frac{\partial Q_{\text{N}}}{\partial \bar{I}} \frac{\text{d} \bar{I}}{\text{d} t} \right)
% \end{equation}
% and reorganizing:
% %intermediate step:
% % \begin{equation}\label{eq:sdin2.5}
...
%  s(\text{DIN}) = \frac{\text{d}\text{DIN}}{\text{d}t} = \frac{F_{\text{DON}-\text{DIN}} - \text{Phy}_{\text{C}} \left(V_N +  \frac{\partial Q_{\text{N}}}{\partial \bar{I}} \frac{\text{d} \bar{I}}{\text{d} t} \right)}{ 1+\text{Phy}_{\text{C}}\frac{\partial Q_{\text{N}}}{\partial \text{DIN}}}
% \end{equation}
...
In Eq.~\eqref{eq:sdin3}, partial derivatives of $Q_{\text{N}}$ with respect to DIN, $\bar{I}$ and $L_{\text{D}}$ are obtained by canonical application of the chain rule. As in \citet{Smith2016}, defining $Z = (Q_{\text{N,0}}/2)\left(\hat{\mu}_{\text{net}}/\hat{V}_{\text{N}}+\zeta_{\text{N}} \right)$ in Eq.~\eqref{eq:Qopt}:
\begin{flalign}\label{eq.delQdelN}
 ...
 \label{eq.delQdd} 
 \frac{\partial Q_{\text{N}}}{\partial L_{\text{D}}} &= \frac{\partial Q_{\text{N}}}{\partial Z} \frac{\partial Z}{\partial L_{\text{D}}}
\end{flalign}
...
\begin{flalign}
  \hat{V}_{\text{N}} &= \frac{(1-f_{\text{A}})\hat{V}_0 f_{\text{A}} \hat{A}_0 \text{DIN}}{(1-f_{\text{A}})\hat{V}_0 + f_{\text{A}} \hat{A}_0 \text{DIN}}
  = \frac{\hat{V}_{0}\cdot \hat{A}_{0}\cdot\text{DIN}}{{\left(\sqrt{\hat{V}_{0}} + \sqrt{\hat{A}_{0}\cdot \text{DIN}}\right)}^{2}}, \qquad
...
For calculating the partial derivative of $Z$ with respect to $\bar{I}$, ${\partial Z}/{\partial \bar{I}}$, we recall $\hat{\mu}_{net}$, $L_{\text{I}}$ and $\hat{\theta}$ from K21, Eqs.~20--23 \& 26:
\begin{flalign}
 ...
 \hat{\mu}_{net} &= L_{\text{D}}\hat{\mu}_0 L_{I} (1-\zeta_{Chl}\hat{\theta})- R^{Chl}_{\text{M}} \zeta_{Chl}\hat{\theta}\\
  L_{\text{I}} &= 1 - \exp \left( \frac{-\alpha \hat{\theta} \bar{I}}{\hat{\mu}_0} \right)\\
  \hat{\theta} &= \frac{1}{\zeta_{\text{Chl}}} + \frac{\hat{\mu}_{0}}{\alpha\cdot \bar{I}} \cdot (1 - W)\\
...
\end{flalign}
where $\mathrm{W}_{0}$ is the 0-branch of Lambert's W-function, and $\alpha$ and $\zeta_{\text{Chl}}$ are model parameters (initial Chl-specific slope of P-I curve and cost of chlorophyll synthesis, respectively, Table~3 in K21). %  $\hat{\theta}$ is the optimal chlorophyll density in the chloroplast.\\
...
\begin{flalign}
 ...
 \frac{\partial Z}{\partial \hat{\mu}_{net}} %\left(
 \frac{\partial \hat{\mu}_{net}}{\partial\bar{I}} \\
%   + \frac{\partial \hat{\mu}_{net}}{\partial\hat{\theta}} \frac{\mathrm{d}\, \hat{\theta}}{\mathrm{d}\,\bar{I}} \right)
 \intertext{where solution for the individual terms can be obtained as}
...
% \frac{\partial \hat{\mu}_{net}}{\partial\hat{\theta}} &= L_{\text{D}} \cdot [ \alpha \cdot \bar{I} \cdot ( 1 - L_{\text{I}} ) \cdot (1 - \zeta_{\text{Chl}} \cdot \hat{\theta}) - L_{\text{I}} \cdot \zeta_{\text{Chl}} \cdot \hat{\mu}_{0} ] - R_{\text{M}}^{\text{Chl}} \cdot \zeta_{\text{Chl}} \\
% \frac{\mathrm{d}\, \hat{\theta}}{\mathrm{d}\, \bar{I}} &= -\frac{\hat{\mu}_{0}}{\alpha\cdot \bar{I}^{2}} (1 - W) - \frac{W}{1 + W} \frac{1}{\bar{I}\cdot \zeta_{\text{Chl}}}
 \intertext{The daylength derivatives are}
 \frac{\partial Z}{\partial L_{\text{D}}} &= \frac{\partial Z}{\partial \hat{\mu}_{\text{net}}} \frac{\partial \hat{\mu}_{\text{net}}}{\partial L_{\text{D}}} \\
 \frac{\partial \hat{\mu}_{\text{net}}}{\partial L_{\text{D}}} &= \left[ \hat{\mu}_{0} \cdot L_{\text{I}} + L_{\text{D}} \cdot \alpha \cdot \bar{I} \cdot (1 - L_{\text{I}})
   \frac{\partial \hat{\theta}}{\partial L_{\text{D}}}\right] (1 - \zeta_{\text{Chl}} \hat{\theta})
 - (L_{\text{D}} \cdot \hat{\mu}_{0} \cdot L_{\text{I}} + R_{\text{M}}^{\text{Chl}}) \cdot \zeta_{\text{Chl}} \frac{\partial \hat{\theta}}{\partial L_{\text{D}}} \\
 \frac{\partial \hat{\theta}}{\partial L_{\text{D}}} &=
 \frac{W}{1 + W} \frac{\hat{\mu}_{0}}{\displaystyle \alpha \cdot \bar{I} \cdot \left( 1 + L_{\text{D}} \frac{\hat{\mu}_{0}}{R_{\text{M}}^{\text{Chl}}} \right)}
\end{flalign}
Note that $\partial{\hat{\mu}_{\text{net}}} / \partial \hat{\theta} = 0$ by definition and is therefore not needed.

We would like to clarify that (1) the DIN in the denominator in the solution provided by S16 (their Eq.~A-5) was a typo; (2) replacement of $\hat{\mu}_g$ (in S16, $\hat{\mu}^I$) with $\hat{\mu}_{net}$ in our model (see K21) results in the appearance of $(1-\hat{\theta} \cdot \zeta_{\text{Chl}})$ when computing $\partial \hat{\mu}_{\text{net}} / \partial \bar{I}$; (3) $L_{\text{D}}$ used to be implicit in S16, now it's explicit (K21 Eq.~21) therefore it appears for $\partial\hat{\mu}_{\text{net}} / \partial \bar{I}$ unlike in S16 \onur{This is what I'm guessing, but maybe it was just forgotten}. %However, since $L_D$ is set to 1 in the current study (Section~\ref{S:DescSetup}), this difference is practically not relevant for our immediate purposes.\\ 

The final terms required in Eq.~\eqref{eq:sdin3} are the changes in $\bar{I}$ and $L_{\text{D}}$ over time, i.e., $\text{d}\,\bar{I} / \text{d}\,t$ and $\text{d}\,L_{\text{D}} / \text{d}\,t$. When the irradiance is supplied externally, as typically the case in realistic coupled physical-biological models it is not possible to obtain its temporal derivative analytically, therefore  it is numerically approximated as the discrete backward difference between its current ($t=i$) and previous ($t=i-1$) values, divided by the size of the integration time step i.e., $\text{d}\, \bar{I} / \text{d}\, t \approx (\bar{I}_{i} - \bar{I}_{i-1}) / \Delta t$ (see also Section~\ref{S:DescT1} designed to test the sensitivity of the model results and conservation behavior to this approximation).
...
\subsubsection{0D setup (T1-T2)}
    ...
    \begin{figure}[ht!]
    \includegraphics[width=14cm,trim=0cm 0mm 0.0cm 0mm, clip]{figures/2022-01-24a_T10_Ld1_ThatVar_PARext_vs_PARint/0D-Highlat_wconst_lext_Ld1_T20_CbasedIA_modular_24h_vs_0D-Highlat_wconst_lint_Ld1_T20_CbasedIA_modular_24h_cont_abio0_2y.png}
...
\begin{figure}[ht!]
\includegraphics[width=14cm,trim=0cm 11mm 0.0cm 5mm, clip]{figures/2022-01-24a_T10_Ld1_ThatVar_PARext_vs_PARint/0D-Highlat_wconst_lext_Ld1_T20_CbasedIA_modular_24h_vs_0D-Highlat_wconst_lint_Ld1_T20_CbasedIA_modular_24h_cont_abio1_2y.png}
...
\begin{figure}[ht!]
\includegraphics[width=14cm,trim=0cm 11mm 0.0cm 5mm, clip]{figures/2022-01-24b_T10_LdVar_ThatVar_PARext_IA_vs_DA/0D-Highlat_wconst_lext_T20_CbasedIA_modular_24h_vs_0D-Highlat_wconst_lext_T20_CbasedDA_modular_24h_cont_abio1_2y.png}

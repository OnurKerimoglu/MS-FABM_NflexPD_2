...
% Here, $\frac{\text{d} Phy_{\text{C}}}{\text{d} t} \big\rvert_G$ corresponds to gross growth of phytoplankton C biomass, i.e., $F_{DIC-Phy_{\text{C}}}$ in Eq.~\eqref{eq:sPhyC}. Substituting therefore Eq.~\eqref{eq:fdicphyc} in Eq.~\eqref{eq:dphyNdt}:
...
% \end{equation}
% where, assuming balanced growth \citep{Burmaster1979}, i.e., $V_N = Q_N \cdot \mu$  (K21, Eq.~6) the first term on the right hand side of Eq.~\eqref{eq:dphyNdt2} can be replaced with $V_N \cdot Phy_{\text{C}}$, i.e., 
...
\end{flalign}
$\mathrm{W}_{0}$ is the 0-branch of Lambert's W-function, and $\alpha$ and $\zeta_{\text{Chl}}$ are model parameters (initial Chl-specific slope of P-I curve and cost of chlorophyll synthesis, respectively, Table~3 in K21). %  $\hat{\theta}$ is the optimal chlorophyll density in the chloroplast.\\

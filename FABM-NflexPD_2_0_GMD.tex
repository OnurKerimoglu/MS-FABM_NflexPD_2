...
\onur{In the N-based version, we had (K21-N, Eq.~5):
\begin{equation}\label{eq:Fdinphy}
  F_{DIN-Phy_{\text{N}}} = V_N \cdot Phy_{\text{C}}
\end{equation}
i.e., excluding the $\frac{\text{d} Q_\text{N}}{\text{d} t}$ term in Eq.~\ref{eq:dphyNdt3}. As we discussed, this doesn't violate the internal consistency of the N-based version, but it is still a curious why we have this different here in this version, so it would be great if we could explain the reasons for the difference.
}

Given that the terms $\hat{V}_\text{N}$ and $\hat{mu}_net$ used for calculating $Q_\text{N}$ (Eq.~\ref{eq:Qopt}) are respectively functions of DIN (Eq.~16 in K21-N), $\bar{I}$ and daytime average irradiance, $\bar{I}$ (Eqs.~20-22 in K21-N), $\frac{\text{d} Q_\text{N}}{\text{d} t}$ in Eq.~\ref{eq:dphyNdt2} :
...
following \citet{Smith2016}, defining $Z = \frac{2}{Q_\text{N,0}{({\hat{\mu}_{\text{net}}}/{\hat{V}_\text{N}}+\zeta_{\text{N}})}}$ in Eq.~\ref{eq:Qopt}, partial derivatives of $Q_\text{N}$ with respect to DIN and $\bar{I}$ can be found as:
\begin{flalign}
...
\begin{flalign}
 ...
 \frac{\partial Z}{\partial \bar{I}} &= \frac{Q_\text{N,0} \cdot \alpha \cdot \hat{\Theta}}{\hat{V}_\text{N} \cdot \text{DIN}} \left ( 1 - L_\text{I} \right) 
\end{flalign}

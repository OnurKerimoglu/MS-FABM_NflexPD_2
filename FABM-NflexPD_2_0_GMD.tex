...
\begin{description}
 ...
 \item [T3] demonstration of a model run with multiple phytoplankton groups.
...
Our results demonstrate that a state variable that tracks the elemental content of plankton can be effectively removed, without leading to major issues in mass balance, in a 0D setup. In comparison to a fully explicit dynamic variant, Removing a state variable in such a setup does not seem to result clear advantages in computational efficiency, for simulating a low number of phytoplankton groups, however when simulating 50 groups, IA was $\approx$10.2\% faster (i.e., real time: 458 s instead of 510 s; see also Fig.~\ref{f.speed}). For simulations with a small number of phytoplankton, any potential reduction in computational costs owed to one less state variable in the instantaneous variant is presumably compensated by the  the additional logic and calculations required (see Appendix-\ref{S:Sol}). However, at a higher number of groups to be simulated, requirements for allocating memory to trace a higher number of state variables is probably outweighing the additional costs associated with flux calculations \onur{these were some quick thoughts, if anyone knows something more about this, or a reference, please let me know}.
...
\codeavailability{For running the model and reproducing the results presented in this study, FABM and GOTM need to be downloaded and installed. See \url{https://github.com/fabm-model/fabm/wiki/GOTM} for the instructions. The version of the FABM-NflexPD used in this manuscript has been stored in Zenodo repository \onur{todo}. Instructions for compiling FABM-NflexPD for GOTM-FABM and a 0D setup are provided in README.md. The `src’ folder contains the Fortran codes. The model was implemented as two separate modules: the `phy.F90' module that describes phytoplankton growth and the `abio.F90' module that describes everything other than phytoplankton (See Fig.~1 in \citet{Kerimoglu2021}). The phytoplankton module can reproduce the behavior of the both IA and DA variants considered in the manuscript through optional parameters. The `testcases’ folder contains the configuration (yaml) file that was used to produce the results presented for T1-T2 in this manuscript, and with two phytoplankton groups.}
...
To facilitate the solutions of the $\partial Q/\partial E$ ($E=\{\mathrm{DIN},\bar{I},\mathrm{T},\mathrm{L}_\mathrm{D}\}$) in Eq.~\eqref{eq:sdin3}, we introduce a new variable $Z$ and re-write Eq.~\eqref{eq:Qopt} in terms of $Z$, as in \citet{Smith2016}:
\begin{flalign}
  \label{eq.Z}
...
\end{flalign}
%The partial derivatives of $T$ Eq.~\eqref{eq:sdin3} are approximated numerically, as the analytical solution of $\partial Q_{\text{N}}/\partial T$ is unwieldy, given the temperature dependence of $\hat{\mu}_0$,  $\hat{V}_0$ and $R_{\text{M}}^{\text{Chl}}$, and numerous occurrence of these terms in intermediate quantities $\hat{\theta}$ and $L_{\text{I}}$ when $Q_{\text{N}}$ (Eq.~\ref{eq:Qopt}) is fully expanded. The numerical difference approximation is achieved by storing the value of $T$ in the previous integration step ($t=i-1$), calculating $Q_{\text{N}}$ based on this previous value of $T$ (but the values of DIN, $\bar{I}$ and $L_{\text{D}}$ in the current time step ($t=i$)), and subtracting this from the current value of $Q_{\text{N}}$, i.e.,
%\begin{align}

...
FABM-NflexPD 2.0 differs from FABM-NflexPD 1.0 (K21) in two major respects.  (1) The new version maintains mass balance for both C and N, instead of only N\@.  (2) We restrict this version to 0D for simplicity, which allows subsuming the DOM compartment in the detritus compartment, leaving 5 state variables to represent dissolved inorganic C and N (DIC and DIN), phytoplankton C ($\text{Phy}_{\text{C}}$), and detritus C and N ($\text{Det}_{\text{C}}$ and $\text{Det}_{\text{N}}$). Moreover, we consider a dilution term for being able to simulate a non-mass-conserving system, such as a chemostat or a surface mixed layer (SML). The rates of change of the state variables are
\begin{flalign}
  \label{eq:dic}
  \frac{\mathrm{d}\, \text{DIC}}{\mathrm{d}\, t} &= \underbrace{F_{\text{Det}_{\text{C}}-\text{DIC}}}_{\textrm{Remineralization}} - \underbrace{F_{\text{DIC}-\text{Phy}_{\text{C}}}}_{\textrm{Net C uptake}} - \underbrace{D \cdot(\text{DIC}-\text{DIC}_{in})}_{\text{Dilution}} \\
  \label{eq:sdin}
  \frac{\mathrm{d}\,\text{DIN}}{\mathrm{d}\,t} &= F_{\text{Det}_{\text{N}}-\text{DIN}} - F_{\text{DIN}-\text{Phy}_{\text{N}}} - D \cdot (\text{DIN}-\text{DIN}_{in}) \\
  \label{eq:sPhyC}
  ...
  \label{eq:sDetC}
  \frac{\mathrm{d}\, \text{Det}_{\text{C}}}{\mathrm{d}\, t} &= F_{\text{Phy}_{\text{C}}-\text{Det}_{\text{C}}} - F_{\text{Det}_{\text{C}}-\text{DIC}}  - D \cdot \text{Det}_{\text{C}} - \underbrace{\frac{w_{Det}}{H_{SML}} \cdot \text{Det}_{\text{C}}}_{\text{Sinking}} \\
  \label{eq:sDetN}
  \frac{\mathrm{d}\, \text{Det}_{\text{N}}}{\mathrm{d}\, t} &= F_{\text{Phy}_{\text{N}}-\text{Det}_{\text{N}}} - F_{\text{Det}_{\text{N}}-\text{DIN}} - D \cdot \text{Det}_{\text{N}} - \frac{w_{Det}}{H_{SML}} \cdot \text{Det}_{\text{N}}
\end{flalign}
...
\end{equation}
where $Q$ is the phytoplankton N quota (N:C ratio). Dilution and sinking terms describe fluxes in and out of the system, and non-zero only for T4 (see below).
...
\begin{table*}[htb]
  ...
  \begin{tabular}{l l c l}
    ...
    DIC$_{in}$   & DIC concentration in the inflow medium  & 1000   & \unit{molC\ m^{-3}} \\
    DIN$_{in}$   & DIN concentration in the inflow medium  & 5   & \unit{molN\ m^{-3}} \\
    $w_{Det}$   & Sinking rate of detritus    & 0 (T1-T3); 0.2 (T4)   & \unit{m\  d^{-1}} \\
    $H_{SML}$   & Height of the SML    & 20   & \unit{m} \\
    \bottomhline
  \end{tabular}
...
\subsection{Test setups and model operation}\label{S:DescSetup}
    ...
    \subsubsection{T4}
    Aim of this test is to test the model in a non-closed system, such as a chemostat system using a non-zero $D$ in Eqs.~\ref{eq:dic}-\ref{eq:sDetN} to represent dilution; or the dynamics within an SML, using non-zero $D$ and $w_{\text{Det}}$, to represent mixing with the layer below the SML and sinking out of the SML, respectively. \onur{do we need to tell about the motivation to include such a test (i.e., that in a closed system, DIN does not need to be dynamically traced, but can be found simply by substracting Q*PhyC, DON and DetN from the initial $\sum{N}$)? Imo we better tell this in the discussion.}
    
    In order to check the consistency of the model in such a setup, we introduce the state variables C$_{out}$ and N$_{out}$, whose sole aim is to trace the dilution and sinking fluxes:
    \begin{flalign}
    \label{eq:Xout}
    \frac{\mathrm{d}\,\text{X}_{out}}{\mathrm{d}\,t} &= D \cdot \left( (\text{DIX}-\text{DIX}_{in}) + \text{Phy}_{\text{X}} + \text{Det}_{\text{X}} \right) + \frac{w_{Det}}{H_{SML}} \cdot \text{Det}_{\text{x}}, \ \text{X}=\{\text{C},\text{N}\}
    \end{flalign}
    such that the when sum of these variables with the respective C and N variables in Eqs.~\ref{eq:dic}-\ref{eq:sDetN}, total N and C should be still conserved, which we demonstrate with a simulation (see Table~\ref{T.pars} for the values of the additional parameters that describe the dilution and sinking fluxes).

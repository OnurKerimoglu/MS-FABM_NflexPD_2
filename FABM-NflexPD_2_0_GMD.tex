...
\affil[2]{Earth SURFACE Research Center, Research Institute for Global Change, JAMSTEC, Japan}
...
\end{flalign}
the partial derivative of $Z$ with respect to DIN can found to be similar to that in S16, with the only exception that their $\hat{\mu}^I$ is replaced by our $\hat{\mu}_{net}$ \onur{I could not reproduce this solution}:
\begin{flalign}
...
\end{flalign}
where $\alpha$ and $\zeta_\text{Chl}$ are model parameters (initial Chl-specific slope of P-I curve and cost of chlorophyll synthesis, respectively, Table~3 in K21) and $L_\text{D}$ is fractional day length (Eq.~21 in K21). %  $\hat{\theta}$ is the optimal chlorophyll density in the chloroplast.\\
\onur{Notes: 1) No DIN in the denominotor of $\frac{\partial Z}{\partial \bar{I}}$ (apparent typo in S16). 2) replacement of $\hat{\mu}_g$ (in S16, $\hat{\mu}^I$) with $\hat{\mu}_{net}$ results in appearance of $(1-\hat{\theta} \cdot \zeta_\text{Chl})$ when computing $\frac{\hat{\mu}_{net}}{\partial L_I}$. 3) Probably $L_\text{D}$ used to be implicit earlier, now it's explicit (K21 eq. 21) therefore it appears for $\frac{\hat{\mu}_{net}}{\partial L_I}$ unlike in S16, but since $L_D$ is set to 1, (Section~\ref{S:DescSetup}), this should not matter.}\\ 
...
\begin{figure}[ht!]
\includegraphics[width=14cm,trim=0cm 0mm 0.0cm 0mm, clip]{figures/2022-01-05_T10_TheHat03_Ld1_PARint_vs_PARext/0D-Highlat_wconst_T10_lext_CbasedIA_TheHat03_Ld1_dm_vs_0D-Highlat_wconst_T10_lint_CbasedIA_TheHat03_Ld1_dm_cont_abio0_2y.png}
...
\begin{figure}[ht!]
\includegraphics[width=14cm,trim=0cm 9mm 0.0cm 5mm, clip]{figures/2022-01-05_T10_TheHat03_Ld1_PARint_vs_PARext/0D-Highlat_wconst_T10_lext_CbasedIA_TheHat03_Ld1_dm_vs_0D-Highlat_wconst_T10_lint_CbasedIA_TheHat03_Ld1_dm_cont_abio1_2y.png}

...
\end{flalign}
In Eqs.~\eqref{eq.delQdelN}-\eqref{eq.delQdelI}, the common term $\frac{\partial Q_{\text{N}}}{\partial Z}$, as in S16, yields \onur{I verified this}:
...
The final term required in Eq.~\eqref{eq:sdin3} is the change in $\bar{I}$ over time, i.e., $\frac{\text{d}\bar{I}}{\text{d}t}$.
...
Both the irradiance and its temporal derivative, as found by the numerical and analytical approaches seem to be identical (Fig.~\ref{f.I}).
...
As shown in Fig.~\ref{f.CNvars}, the model is fully conservative with respect to C and N\@. 

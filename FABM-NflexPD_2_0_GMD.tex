...
\subsection{Test setups and model operation}\label{S:DescSetup}
    ...
    In order to check the consistency of the model in such a setup, we introduce the state variables $\text{Ext}_{\text{C}}$ and $\text{Ext}_{\text{N}}$, purpose of which are to trace the amount of N and C exported from and imported to the system:
    \begin{flalign}
    \label{eq:Xext}
    \frac{\mathrm{d}\,\text{Ext}_{X}}{\mathrm{d}\,t} &= D \cdot \left(\text{Phy}_{\text{X}} + \text{Det}_{\text{X}}  + \text{DOX} + (\text{DIX}-\text{DIX}_{in}) \right) + \frac{w_{Det}}{H_{SML}} \cdot \text{Det}_{\text{X}}\ \text{, X}=\{\text{C},\text{N}\}
    \end{flalign}
    such that the global N and C in the system, that can be obtained by the sum of these variables with the respective C and N variables in Eqs.~\ref{eq:sPhyC}-\ref{eq:dim}, should be ideally conserved. We examine the consistency of the model in this regard with a simulation (see Table~\ref{T.pars} for the values of the additional parameters that describe the dilution and sinking fluxes).
